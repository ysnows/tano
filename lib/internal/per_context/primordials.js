...
function getNewKey(key) {
  if (typeof key === 'symbol') {
    // Access description safely — stock JSC may throw if `this` binding
    // is lost through uncurryThis/ReflectApply chains.
    let desc;
    try {
      desc = key.description;
    } catch {
      const str = Symbol.prototype.toString.call(key);
      const m = str.match(/^Symbol\((.+)\)$/);
      desc = m ? m[1] : 'unknown';
    }
    return `Symbol${desc[7].toUpperCase()}${desc.slice(8)}`;
  }
  return `${key[0].toUpperCase()}${key.slice(1)}`;
}

function copyAccessor(dest, prefix, key, { enumerable, get, set }, originalKey) {
  // In JavaScriptCore, some built-in getters (e.g., Symbol.prototype.description)
  // reject `this` values passed through uncurryThis/Function.prototype.call
  // even when the `this` is the correct type. Work around by falling back to
  // direct property access using the original property key.
  const uncurried = uncurryThis(get);
  const propKey = originalKey !== undefined ? originalKey : key;
  const safeGet = function(thisArg) {
    try {
      return uncurried(thisArg);
    } catch {
      return thisArg[propKey];
    }
  };
...
function copyPropsRenamed(src, dest, prefix) {
  for (const key of ReflectOwnKeys(src)) {
    ...
    if ('get' in desc) {
      copyAccessor(dest, prefix, newKey, desc, key);
    } else {
