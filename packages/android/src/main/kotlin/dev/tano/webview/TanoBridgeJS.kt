...
/**
 * Contains the JavaScript bridge code injected into the Android WebView
 * at page start.
 *
 * This creates the `window.Tano` API that the user's web app uses to
 * communicate with native plugins via `invoke()`, listen for events
 * via `on()`, and send fire-and-forget messages via `send()`.
 *
 * On Android, the native message posting uses `TanoAndroid.invoke()`
 * (via addJavascriptInterface) instead of the iOS
 * `window.webkit.messageHandlers.tano.postMessage()`.
 *
 * Mirrors the iOS TanoBridgeJS.
 */
object TanoBridgeJS {

    /**
     * The bridge JavaScript source, injected into the WebView at document start.
     *
     * Provides:
            ...
            window.Tano = {
                ...
                invoke: function(plugin, method, params) {
                    return new Promise(function(resolve, reject) {
                        ...
                        TanoAndroid.invoke(JSON.stringify({
                            type: 'invoke',
                            callId: callId,
                            plugin: plugin,
                            method: method,
