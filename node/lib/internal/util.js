...
function assignFunctionName(name, fn, descriptor = kEmptyObject) {
  if (typeof name !== 'string') {
    // SymbolPrototypeGetDescription may throw in JavaScriptCore where
    // uncurryThis(getter).call(symbol) rejects the this binding.
    let symbolDescription;
    try {
      symbolDescription = SymbolPrototypeGetDescription(name);
    } catch {
      if (typeof name === 'symbol') {
        symbolDescription = name.description;
        if (symbolDescription === undefined) {
          const str = Symbol.prototype.toString.call(name);
          const m = str.match(/^Symbol\((.+)\)$/);
          symbolDescription = m ? m[1] : 'Symbol';
        }
      }
    }
    if (symbolDescription === undefined) symbolDescription = 'Symbol';
